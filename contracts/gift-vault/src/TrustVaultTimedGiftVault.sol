...
contract TrustVaultTimedGiftVault {
    ...
    function createGift(address recipient, uint256 amount, uint64 unlockTimestamp)
        external
        nonReentrant
        returns (uint256 giftId)
    {
        ...
        gifts[giftId] = Gift({
            sender: msg.sender, recipient: recipient, amount: amount, unlockTimestamp: unlockTimestamp, claimed: false
        });

        _safeTransferFrom(usdc, msg.sender, address(this), amount);

        emit GiftCreated(giftId, msg.sender, recipient, amount, unlockTimestamp);
    }
    ...
    function claim(uint256 giftId) external nonReentrant {
        ...
        _safeTransfer(usdc, gift.recipient, gift.amount);

        emit GiftClaimed(giftId, gift.recipient, gift.amount, block.timestamp);
    }
    ...
    function isClaimable(uint256 giftId) external view returns (bool) {
        ...
        return gift.sender != address(0) && !gift.claimed && block.timestamp >= gift.unlockTimestamp;
    }
