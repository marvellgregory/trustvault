...
test("Swap answer states Coming Soon and offers no execution", async () => {
  ...
  assert.equal(plan.confidence, "VERIFIED");
  assert.match(plan.answer, /Coming Soon/i);
  assert.match(
    plan.answer,
...
test("wallet and Gift Vault explanations stay grounded in TrustVault sources", async () => {
  ...
  assert.match(
    wallet.answer,
    /signing authority inside your wallet/i,
  );
  ...
  assert.match(
    gift.answer,
    /create and manage programmable USDC gifts/i,
  );
});
...
test("unknown questions remain unavailable and use the no-guess contract", async () => {
  ...
  assert.match(plan.answer, /won't make up an answer/i);
  assert.match(
    plan.answer,
    /help you approach it another way/i,
  );
});
