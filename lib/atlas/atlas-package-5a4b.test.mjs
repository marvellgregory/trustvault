...
test("5A.4B: Gift Vault start response preserves wallet approval boundary", () => {
  ...
  assert.ok(
    response.actions.every(
      (action) => action.type === "navigate" || action.type === "ask-atlas",
    ),
  );
});
...
test("5A.4B: related features become contextual navigation actions", () => {
  ...
  const navigationActions = response.actions.filter(
  ...
  assert.ok(
    response.actions.some((action) => action.type === "ask-atlas"),
  );
});

test("5A.4B + 5A.4C: feature actions remain non-transactional", () => {
  const featureIds = [
    "marketplace",
    "marketplace-order",
    "delivery-tracking",
  ...
  for (const featureId of featureIds) {
    ...
    assert.ok(
      ...
      `${featureId} exposed an unsafe action type`,
    );
  }
});
