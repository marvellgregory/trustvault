...
test("5B.1G ambiguous explicit orders do not silently narrow to the first ID", () => {
  const entities = [
    {
      kind: "marketplace-order",
      reference: "explicit",
  ...
  assert.deepEqual(
    ...
    {
      query: "show order TV-1001 and order TV-2002",
    },
  );
});

test("5B.1G ambiguous explicit gifts do not silently select the first Gift ID", () => {
  const entities = [
    {
      kind: "gift",
      reference: "explicit",
...
test("5B.1G cross-kind entities still narrow to the tool-relevant explicit entity", () => {
  const entities = [
    {
      kind: "marketplace-order",
      reference: "explicit",
