...
const {
  ...
  restoreUnverifiedWalletConnection,
  verifyWalletProviderIdentity,
...
function connected(chainId = 1, provider = identity()) {
  ...
  return verifyWalletProviderIdentity(
    connectWalletSession(session, { address: ACCOUNT_A, chainId }),
    provider.registryId,
  );
}
...
test("detects Wagmi and Viem account mismatches", () => {
  ...
  const session = verifyWalletProviderIdentity(connectWalletSession(
...
test("automatic reconnect is identity-unverified even on Arc", () => {
  const session = restoreUnverifiedWalletConnection(selectWalletProvider(detected(), identity()), {
    address: ACCOUNT_A,
    chainId: ARC_CHAIN_ID,
  });
  assert.equal(session.state, "IDENTITY_UNVERIFIED");
  assert.equal(session.chain.arcReady, false);
  assert.throws(() => establishArcReadiness(session), /Arc readiness/);
  const evaluated = evaluateWalletCapabilities(session, compatibleCapabilities);
  assert.equal(evaluated.state, "IDENTITY_UNVERIFIED");
  const qualified = evaluateWalletQualification(evaluated, {
    status: "QUALIFIED",
    providerIdentityKey: evaluated.provider.registryId,
    reasons: [],
  });
  assert.equal(qualified.state, "IDENTITY_UNVERIFIED");
});

test("disconnect clears identity verification and snapshots contain no provider object", () => {
  const restored = restoreUnverifiedWalletConnection(selectWalletProvider(detected(), identity()), {
    address: ACCOUNT_A,
    chainId: ARC_CHAIN_ID,
  });
  const disconnected = disconnectWalletSession(restored);
  assert.deepEqual(disconnected.identityVerification, { status: "UNVERIFIED", reason: "NOT_CONNECTED" });
  const snapshot = createSerializableWalletSessionSnapshot(restored);
  assert.equal(snapshot.identityVerification.status, "UNVERIFIED");
  assert.equal("bindings" in snapshot, false);
  assert.doesNotMatch(JSON.stringify(snapshot), /privateKey|seedPhrase|signature|request/);
});

test("Arc readiness alone is not Circle readiness", () => {
  const ready = establishArcReadiness(connected(ARC_CHAIN_ID));
  assert.equal(ready.circleEvidence.status, "CIRCLE_UNBOUND");
  const circleReady = bindCircleEvidence(ready, { providerIdentityKey: ready.provider.registryId, account: ACCOUNT_A, chainId: ARC_CHAIN_ID, bindingGeneration: "generation-1" });
  assert.equal(circleReady.circleEvidence.status, "CIRCLE_READY");
});

test("identity-unverified sessions cannot bind Circle and snapshots sanitize evidence", () => {
  const unverified = restoreUnverifiedWalletConnection(selectWalletProvider(detected(), identity()), { address: ACCOUNT_A, chainId: ARC_CHAIN_ID });
  assert.throws(() => bindCircleEvidence(unverified, { providerIdentityKey: unverified.provider.registryId, account: ACCOUNT_A, chainId: ARC_CHAIN_ID, bindingGeneration: "generation-1" }), /Circle readiness/);
  const snapshot = createSerializableWalletSessionSnapshot(unverified);
  assert.equal(snapshot.circleEvidence.status, "CIRCLE_UNBOUND");
  assert.equal("bindings" in snapshot, false);
  assert.doesNotMatch(JSON.stringify(snapshot), /provider\.request|"adapter"\s*:|"connector"\s*:|"registry"\s*:/i);
});
